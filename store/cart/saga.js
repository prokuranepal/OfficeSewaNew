...
function* addAddressSaga(payload) {
    try {
        const { address, saveAdd } = payload;
        // console.log("saga address value",address, saveAdd, payload)
        // Store
        saveAdd?localStorage.setItem("shippingAdd", JSON.stringify(address)):null;
        yield put(updateShippingAddress(address))
    } catch (err) {
...
function* updateShipFromLocal() {
    try {
        // Store
        const shipA=JSON.parse(localStorage.getItem("shippingAdd"));
        console.log("shipping from local", shipA)
        shipA? yield put(updateShippingAddress(shipA)):null;
    } catch (err) {
        console.log("saga error",err)
        yield put(getCartError(err));
    }
...
function* addItemSaga(payload) {
    try {
        ...
        console.log("saga product value",product)
...
export default function* rootSaga() {
    ...
    yield all([takeEvery(actionTypes.ADD_SHIPPING_ADDRESS, addAddressSaga)]);
    yield all([takeEvery(actionTypes.UPDATE_SHIPADD_FROM_LOCALSTORAGE, updateShipFromLocal)]);
